...
FG = rm.open_resource('GPIB0::19::INSTR')
FG.write('*CLS')
FG.write('*RST')
...
daq_server = zhinst.core.ziDAQServer(server_host, server_port, api_level)
...
def Lock_In_measure(volt, freq):
    ...
    Vpk = volt
    if AC_on != 1:
        FG.write("APPL:DC DEF, DEF, "+ str(offset)+ "V")
    elif AC_on == 1:
        FG.write("APPL:SIN "+str(freq)+" HZ, "+str(Vpk)+" VPP, "+ str(offset)+ "V")
