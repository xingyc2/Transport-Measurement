...
class MeasurementDevice:
    ...
    def connect(self):
        ...
        """
        try:
            self.instrument = visa.ResourceManager().open_resource(self.visa_address)
    ...
    def set_values(self, configuration):
        ...
        """
        try:
            self.instrument.write(f"CONFIGURE {configuration}")  # Replace with the actual command for setting values
...
if __name__ == "__main__":
    device = MeasurementDevice("TCPIP0::192.168.1.100::inst0::INSTR")

    try:
        device.connect()

        # Set configuration values
        configuration_settings = "SAMPLE_RATE=100"
